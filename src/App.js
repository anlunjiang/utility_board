...
const App = () => {
  ...
  let content = (
    <p style={{ textAlign: "center" }}>
      No Items found. Did we buy everything?!
    </p>
  );

  if (itemList.length > 0) {
    content = (
      <ItemList itemsList={itemList} onDeleteItem={deleteItemHandler} />
    );
  }
  ...
  return (
    <div>
      ...
      <section id="items">{content}</section>
    </div>
  );
};
